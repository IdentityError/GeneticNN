...
// private void Start() { fittestData =
// TSaveManager.GetInstance().LoadPersistentData(TSaveManager.CHOSEN_ONE).GetData<CarIndividualData>(); StartSimulation();

// uiManager?.UpdateTrackLength(CalculateTrackLength());
// uiManager?.DrawNetUI(fittestData.GetDNA().topology); uiManager?.AppendToLog("Evaluating the
// current chosen one"); }

// private void Update() { Time.timeScale = timeScale; if (Input.GetKeyDown(KeyCode.R)) {
// StartSimulation(); } }

// private void StartSimulation() { if (fittest != null) { Destroy(fittest.gameObject); }
